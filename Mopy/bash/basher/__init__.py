...
class SaveList(balt.UIList):
    ...
    def OnLabelEdited(self, is_edit_cancelled, evt_label, evt_index, evt_item):
        ...
        selected = [s for s in self.GetSelected() if
                    not bosh.bak_file_pattern.match(s)] # YAK !
        to_select = set()
        to_del = set()
...
class InstallersList(balt.UIList):
    ...
    def _OnEditLabelChar(self, is_f2_down, ec_value, uilist_ctrl):
        ...
        if is_f2_down:
            ...
            selectedText = (ec_value[selection_span[0]:selection_span[1]])
            textNextLower = body_(selectedText)
    ...
    def SetFile(self, fileName=u'SAME'):
        ...
        if fileName:
            ...
            if not installer.espms:
                ...
                names.sort(key=lambda x: cext_(x) != u'.esm')
                names_ = [[u'', u'*'][installer.isEspmRenamed(x)] +
                          x.replace(u'&', u'&&') for x in names]
...
class InstallersPanel(BashTab):
    ...
    def _refresh_installers_if_needed(self, canCancel, fullRefresh,
        ...
        if self.frameActivated:
            omds = [inst_path for inst_path in installers_paths if
                    cext_(inst_path) == u'.omod']
            if any(inst_path not in omods.failedOmods for inst_path in omds):
                self.__extractOmods(omds) ##: cahnge above to all?
    ...
    def __extractOmods(self, omds):
        with balt.Progress(_(u'Extracting OMODs...'),
            ...
            omods = [dirInstallersJoin(x) for x in omds]
    ...
    def OnLabelEdited(self, is_edit_cancelled, evt_label, evt_index, evt_item):
        ...
        with BusyCursor():
            ...
            for screen in selected:
                newName = os.path.normpath(root + numStr + screen.ext) ##: left from GPath - needed?
